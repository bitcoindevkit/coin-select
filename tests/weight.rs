#![allow(clippy::zero_prefixed_literal)]

mod common;
use bdk_coin_select::{Candidate, CoinSelector, Drain, DrainWeights, TargetOutputs};
use bitcoin::{consensus::Decodable, ScriptBuf, Transaction};

fn hex_val(c: u8) -> u8 {
    match c {
        b'A'..=b'F' => c - b'A' + 10,
        b'a'..=b'f' => c - b'a' + 10,
        b'0'..=b'9' => c - b'0',
        _ => panic!("invalid"),
    }
}

// Appears that Transaction has no from_str so I had to roll my own hex decoder
pub fn hex_decode(hex: &str) -> Vec<u8> {
    let mut bytes = Vec::with_capacity(hex.len() * 2);
    for hex_byte in hex.as_bytes().chunks(2) {
        bytes.push(hex_val(hex_byte[0]) << 4 | hex_val(hex_byte[1]))
    }
    bytes
}

#[test]
fn segwit_one_input_one_output() {
    // FROM https://mempool.space/tx/e627fbb7f775a57fd398bf9b150655d4ac3e1f8afed4255e74ee10d7a345a9cc
    let tx_bytes = hex_decode("");
    let tx = Transaction::consensus_decode(&mut tx_bytes.as_slice()).unwrap();
    let input_values = vec![563_336_755];

    let candidates = tx
        .input
        .iter()
        .zip(input_values)
        .map(|(txin, value)| Candidate {
            value,
            weight: txin.segwit_weight().to_wu() as u32,
            input_count: 1,
            is_segwit: true,
        })
        .collect::<Vec<_>>();

    let target_ouputs = TargetOutputs {
        value_sum: tx.output.iter().map(|output| output.value.to_sat()).sum(),
        weight_sum: tx
            .output
            .iter()
            .map(|output| output.weight().to_wu() as u32)
            .sum(),
        n_outputs: tx.output.len(),
    };

    let mut coin_selector = CoinSelector::new(&candidates, common::c_to_c);
    coin_selector.select_all();

    assert_eq!(
        coin_selector.weight(target_ouputs, DrainWeights::NONE),
        tx.weight().to_wu() as u32
    );
    assert_eq!(
        (coin_selector
            .implied_feerate(target_ouputs, Drain::NONE)
            .unwrap()
            .as_sat_vb()
            * 10.0)
            .round(),
        60.2 * 10.0
    );
}

#[test]
fn segwit_two_inputs_one_output() {
    // FROM https://mempool.space/tx/37d2883bdf1b4c110b54cb624d36ab6a30140f8710ed38a52678260a7685e708
    let tx_bytes = hex_decode("");
    // let mut cursor = std::io::Cursor::new(&mut tx_bytes);
    let tx = Transaction::consensus_decode(&mut tx_bytes.as_slice()).unwrap();
    let input_values = vec![003_194_967, 000_014_068];

    let candidates = tx
        .input
        .iter()
        .zip(input_values)
        .map(|(txin, value)| Candidate {
            value,
            weight: txin.segwit_weight().to_wu() as u32,
            input_count: 1,
            is_segwit: true,
        })
        .collect::<Vec<_>>();

    let mut coin_selector = CoinSelector::new(&candidates, common::c_to_c);

    let target_ouputs = TargetOutputs {
        value_sum: tx.output.iter().map(|output| output.value.to_sat()).sum(),
        weight_sum: tx
            .output
            .iter()
            .map(|output| output.weight().to_wu() as u32)
            .sum(),
        n_outputs: tx.output.len(),
    };

    coin_selector.select_all();

    assert_eq!(
        coin_selector.weight(target_ouputs, DrainWeights::NONE),
        tx.weight().to_wu() as u32
    );
    assert_eq!(
        (coin_selector
            .implied_feerate(target_ouputs, Drain::NONE)
            .unwrap()
            .as_sat_vb()
            * 10.0)
            .round(),
        58.1 * 10.0
    );
}

#[test]
fn legacy_three_inputs() {
    // FROM https://mempool.space/tx/5f231df4f73694b3cca9211e336451c20dab136e0a843c2e3166cdcb093e91f4
    let tx_bytes = hex_decode("");
    let tx = Transaction::consensus_decode(&mut tx_bytes.as_slice()).unwrap();
    let orig_weight = tx.weight();
    let input_values = vec![022_680_000, 006_558_175, 006_558_200];
    let candidates = tx
        .input
        .iter()
        .zip(input_values)
        .map(|(txin, value)| Candidate {
            value,
            weight: txin.legacy_weight().to_wu() as u32,
            input_count: 1,
            is_segwit: false,
        })
        .collect::<Vec<_>>();

    let target_ouputs = TargetOutputs {
        value_sum: tx.output.iter().map(|output| output.value.to_sat()).sum(),
        weight_sum: tx
            .output
            .iter()
            .map(|output| output.weight().to_wu() as u32)
            .sum(),
        n_outputs: tx.output.len(),
    };

    let mut coin_selector = CoinSelector::new(&candidates, common::c_to_c);
    coin_selector.select_all();

    assert_eq!(
        coin_selector.weight(target_ouputs, DrainWeights::NONE),
        orig_weight.to_wu() as u32
    );
    assert_eq!(
        (coin_selector
            .implied_feerate(target_ouputs, Drain::NONE)
            .unwrap()
            .as_sat_vb()
            * 10.0)
            .round(),
        99.2 * 10.0
    );
}

#[test]
fn legacy_three_inputs_one_segwit() {
    // FROM https://mempool.space/tx/5f231df4f73694b3cca9211e336451c20dab136e0a843c2e3166cdcb093e91f4
    // Except we change the middle input to segwit
    let tx_bytes = hex_decode("");
    let mut tx = Transaction::consensus_decode(&mut tx_bytes.as_slice()).unwrap();
    tx.input[1].script_sig = ScriptBuf::default();
    tx.input[1].witness = vec![
        // semi-realistic p2wpkh spend
        hex_decode("3045022100bdc115b86e9c863279132b4808459cf9b266c8f6a9c14a3dfd956986b807e3320220265833b85197679687c5d5eed1b2637489b34249d44cf5d2d40bc7b514181a5101"),
        hex_decode("02077741a668889ce15d59365886375aea47a7691941d7a0d301697edbc773b45b"),
    ].into();
    let input_values = vec![022_680_000, 006_558_175, 006_558_200];
    let candidates = tx
        .input
        .iter()
        .zip(input_values)
        .enumerate()
        .map(|(i, (txin, value))| {
            let is_segwit = i == 1;
            Candidate {
                value,
                weight: if is_segwit {
                    txin.segwit_weight()
                } else {
                    txin.legacy_weight()
                }
                .to_wu() as u32,
                input_count: 1,
                is_segwit,
            }
        })
        .collect::<Vec<_>>();

    let target_ouputs = TargetOutputs {
        value_sum: tx.output.iter().map(|output| output.value.to_sat()).sum(),
        weight_sum: tx
            .output
            .iter()
            .map(|output| output.weight().to_wu() as u32)
            .sum(),
        n_outputs: tx.output.len(),
    };

    let mut coin_selector = CoinSelector::new(&candidates, common::c_to_c);
    coin_selector.select_all();

    assert_eq!(
        coin_selector.weight(target_ouputs, DrainWeights::NONE),
        tx.weight().to_wu() as u32
    );
}
