...
- **Breaking:** The read-only query methods (`selected_value`, `input_weight`, `weight`, `excess` and its `rate_`/`absolute_`/`replacement_` variants, `is_funded`/`is_funded_with_drain`, `is_fundable`, `is_within_max_weight`, `fee`, `implied_fee`, `implied_feerate`, `missing`, `drain_value`, `drain`, `effective_value`, `waste`) move off `CoinSelector` onto the new public `SelectionView`, obtained via `CoinSelector::compute_view()` (e.g. `cs.excess(..)` becomes `cs.compute_view().excess(..)`). `BnbMetric::{score, bound, drain}` now receive `&SelectionView` instead of `&CoinSelector`. During branch-and-bound the view's running aggregates are maintained incrementally (delta-aware), so metric evaluation is O(1) per query instead of O(|selected|) — a large speedup at scale.
