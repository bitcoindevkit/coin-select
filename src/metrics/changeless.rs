use crate::{bnb::BnbMetric, float::Ordf32, Drain, SelectionView, Target};
...
impl<M: BnbMetric> Changeless<M> {
    ...
    fn change_unavoidable(&mut self, view: &SelectionView<'_>, target: Target) -> bool {
        if self.0.drain(view, target).is_none() {
            return false;
        }

        let mut least_excess = view.selector().clone();
        view.unselected()
            .rev()
        ...
        self.0.drain(&least_excess.compute_view(), target).is_some()
    }
}
...
impl<M: BnbMetric> BnbMetric for Changeless<M> {
    ...
    fn score(&mut self, view: &SelectionView<'_>, target: Target) -> Option<Ordf32> {
        ...
        if self.0.drain(view, target).is_some() {
            return None;
        }
        self.0.score(view, target)
    }

    fn bound(&mut self, view: &SelectionView<'_>, target: Target) -> Option<Ordf32> {
        if self.change_unavoidable(view, target) {
